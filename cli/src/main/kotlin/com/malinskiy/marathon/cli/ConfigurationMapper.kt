...
class ConfigurationMapper {
    fun map(input: Configuration): MarathonConfiguration {
        return MarathonConfiguration(
            ...
            testClassRegexes = input.testClassRegexes.map { it.toRegex() },
            includeSerialRegexes = input.includeSerialRegexes.map { it.toRegex() },
            excludeSerialRegexes = input.excludeSerialRegexes.map { it.toRegex() },
    ...
    private fun AnalyticsConfiguration.toMarathonAnalyticsConfig(): MarathonAnalyticsConfiguration {
        return when (this) {
            is AnalyticsConfiguration.Disabled -> MarathonAnalyticsConfiguration.DisabledAnalytics
            is AnalyticsConfiguration.Graphite -> MarathonAnalyticsConfiguration.GraphiteConfiguration(this.host, this.port, this.prefix)
            is AnalyticsConfiguration.InfluxDb -> MarathonAnalyticsConfiguration.InfluxDbConfiguration(
                this.url,
                this.user,
                this.password,
                this.dbName,
