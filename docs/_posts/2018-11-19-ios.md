...
## xcResultBundlePath
Use this option to specify the path to store the iOS Test Result i.e. xcResultBundlePath. Default is `derived-data/test/result`.
Please note that since Marathon can run on multiple simulators, each time a new `.xcresult` file will be created 
with timestamp . For example `result_2022-10-28-02-59-50.xcresult`
```yaml
xcResultBundlePath: "derived-data/test/result"
```
