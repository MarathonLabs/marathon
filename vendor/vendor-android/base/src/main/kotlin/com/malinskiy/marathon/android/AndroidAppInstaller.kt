...
class AndroidAppInstaller(configuration: Configuration) {
    ...
    suspend fun prepareInstallation(device: AndroidDevice) {
        val applicationInfo = ApkParser().parseInstrumentationInfo(androidConfiguration.testApplicationApk)
        logger.debug { "Installing application output to ${device.serialNumber}" }
        androidConfiguration.applicationApk?.let {
        ...
        reinstall(device, applicationInfo.instrumentationPackage, androidConfiguration.testApplicationApk)
