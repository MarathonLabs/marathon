    ...
    private fun createRemoteCommandExecutor(transport: Transport.Ssh): SshjCommandExecutor? {
        return try {
            ...
            val sshAuthentication = when (authConfig) {
                is SshAuthentication.PasswordAuthentication -> com.malinskiy.marathon.ios.cmd.remote.ssh.sshj.auth.SshAuthentication.PasswordAuthentication(
                    authConfig.username,
                    authConfig.password
                )

                is SshAuthentication.PublicKeyAuthentication -> com.malinskiy.marathon.ios.cmd.remote.ssh.sshj.auth.SshAuthentication.PublicKeyAuthentication(
                    authConfig.username,
                    authConfig.key
                )
