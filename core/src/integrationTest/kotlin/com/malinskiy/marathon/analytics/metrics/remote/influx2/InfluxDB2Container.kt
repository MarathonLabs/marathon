...
class InfluxDBContainerV2(imageName: DockerImageName) : GenericContainer<InfluxDBContainerV2?>(imageName) {
    ...
    constructor(imageName: String = "influxdb") : this(DockerImageName.parse(imageName))

    init {
        imageName.assertCompatibleWith(DEFAULT_IMAGE_NAME)
        waitStrategy = WaitAllStrategy().withStrategy(
            Wait.forHttp("/ping").withBasicCredentials(username, password).forStatusCode(NO_CONTENT_STATUS_CODE)
    ...
    override fun configure() {
        ...
        retention?.let { ret -> addEnv("DOCKER_INFLUXDB_INIT_RETENTION", ret) }
        adminToken?.let { token -> addEnv("DOCKER_INFLUXDB_INIT_ADMIN_TOKEN", token) }
    }
     ...
     */
    val url by lazy { "http://" + this.host + ":" + getMappedPort(INFLUXDB_PORT) }
